...
#define DEFINE_EXTERNAL_FUNCTION(_definition, _addr_e, _addr_j, _addr_p, _addr_k) \
    asm _definition { \
        nofralloc; \
        mflr r12; \
        bl port_call; \
...
static void* loadCustomModule(EGG::Heap* heap)
{
    ...
    snprintf(relPath, 64, "/rel/patch_%c.rel",
    ...
    snprintf(relPath, 64, "/xoris/rel/patch_%c.rel",
             *reinterpret_cast<char*>(0x80000003));
    const s32 fd = IOS_Open(relPath, 1);
    if (fd < 0)
        return 0;
    
    void* data = loadFromNAND(heap, fd);
    IOS_Close(fd);
    return data;
}
...
{
    OSReport("xoris loader assertion failed\n"
             "  %s\n"
             "  at file %s, line %d\n", expr, file, line);
    while (1) { }
}
